...
public class TransactionDAO {
    ...
    public Transaction createTransaction(Integer fromAccountID, Integer toAccountID, BigDecimal amount, String transactionType, String transactionContent) throws SQLException {
    ...
    // [TẠO GIAO DỊCH - TRANSACTION] Hàm này dùng chung kết nối (Connection) để đảm bảo tính toàn vẹn dữ liệu (Rollback nếu lỗi)
    ...
    // [LỊCH SỬ THEO USERNAME] Lấy tất cả giao dịch của một người dùng (dựa vào Username)
    public List<Transaction> getTransactionsByUsername(String username) throws SQLException {
        List<Transaction> list = new ArrayList<>();
        // Sử dụng Subquery để tìm tất cả account_id thuộc về username đó, tránh trùng lặp
    ...
    // [THỐNG KÊ] Đếm tổng số giao dịch của một khách hàng
    public int getTransactionCountByCustomerId(int customerId) throws SQLException {
        // Dùng COUNT(DISTINCT) để tránh đếm trùng 1 giao dịch 2 lần (vừa là người gửi vừa là người nhận)
        String sql = "SELECT COUNT(DISTINCT transaction_id) FROM transactions t " +
                     "WHERE t.from_account_id IN (SELECT account_id FROM accounts WHERE customer_id = ?) " +
                     "OR t.to_account_id IN (SELECT account_id FROM accounts WHERE customer_id = ?)";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    // [THỐNG KÊ] Tổng số lượng giao dịch toàn hệ thống
    ...
    // [THỐNG KÊ] Đếm giao dịch theo loại (Deposit, Withdraw, Transfer)
    public int getTransactionCountByType(String type) throws SQLException {
    ...
    // [QUẢN TRỊ] Xóa một giao dịch (Dùng cẩn thận)
